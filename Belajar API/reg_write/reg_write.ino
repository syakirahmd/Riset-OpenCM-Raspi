...
const uint16_t SR_START_ADDR = 36;//present position
...
void setup() {
  ...
  sw_data[0].goal_posisiAX = 512;
  sw_data[1].goal_posisiAX = 512;
  for(i=0; i<DXL_ID_CNT; i++){
...
void loop() {
  ...
  for(i = 0; i < DXL_ID_CNT; i++){
    sw_data[i].goal_posisiAX = sw_data[i].goal_posisiAX;
    } sw_infos.is_info_changed = true;
  ...
  if(dxl.syncWrite(&sw_infos) == true){
    ...
    for(i=0; i<sw_infos.xel_count; i++){
      ...
      DEBUG_SERIAL.print("\t Goal Velocity: ");DEBUG_SERIAL.println(sw_data[i].goal_posisiAX);
    }
  }else{
